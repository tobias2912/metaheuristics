...
def __get_car_index2(car_number, solution, num_cars):
    """
    deprecated
    """
    assert 1 <= car_number <= num_cars + 1
    carIndex = 1
    start = None
    found = False
    ...
    # TODO: terrible stuff
    if start is None:
        # return len(solution), len(solution)+1
        raise BaseException(solution, car_number, num_cars)
    return start, stop
